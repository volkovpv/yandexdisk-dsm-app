...
# «Синхронизировать сейчас». Запускает тот же sync, что и Планировщик задач, но ОТВЯЗАННЫМ
# от HTTP-запроса процессом и отвечает СРАЗУ — НЕ дожидаясь конца bisync. Почему: полный
# bisync к Яндекс Диску (особенно первый --resync) легко дольше таймаута веб-стека DSM.
# Синхронный вызов (yd sync; ждём итог) отдавал не-JSON (страница таймаута) -> UI показывал
# «Некорректный ответ сервера», а synoscgi мог убить процесс-группу CGI по таймауту, оборвав
# сам прогон. Detach (setsid -> nohup -> подоболочка) с закрытым stdin и всем выводом в
# /dev/null: процесс НЕ держит pipe stdout CGI (иначе webman ждал бы EOF и таймаут вернулся
# бы) и переживает закрытие запроса — ровно как запуск из Планировщика задач. Зовём "$YD_BIN"
# напрямую (не функцию yd(): setsid/nohup исполняют внешнюю команду, не шелл-функцию). Итог UI
# узнаёт опросом status.cgi (sync.state обновляется в конце прогона); подробности — в rclone.log.
do_sync() {
    if command -v setsid >/dev/null 2>&1; then
        setsid "$YD_BIN" sync >/dev/null 2>&1 </dev/null &
    elif command -v nohup >/dev/null 2>&1; then
        nohup "$YD_BIN" sync >/dev/null 2>&1 </dev/null &
    else
        ( "$YD_BIN" sync >/dev/null 2>&1 </dev/null & ) &
    fi
    json_ok_msg "Синхронизация запущена"
}
